"""
NeuroTrade — Technical Indicators
indicators.py — Compute all technical indicators on OHLCV DataFrames.

Column contract (used by app.py, ml_models.py, dl_models.py, quantum_models.py):
...
def add_all_indicators(df):
    """Enrich an OHLCV DataFrame with all supported indicators."""
    df = df.copy()
    df = add_sma(df)
    df = add_ema(df)
    df = add_macd(df)
