...
def add_macd(df):
    ...
    df['MACD']        = ema12 - ema26
    # FIX: app.py expects 'MACD_Signal' (capital S)
...
def add_adx(df, period=14):
    ...
    # FIX: app.py expects 'DI_Pos' and 'DI_Neg'
...
def add_stochastic(df, period=14):
    ...
    # FIX: app.py expects 'Stoch_K' and 'Stoch_D'
    df['Stoch_K'] = k
...
def add_bollinger_bands(df, period=20):
    # FIX: app.py expects 'BB_Mid' not 'BB_Middle'
...
def add_obv(df):
    # FIX: vectorised — avoids index-based loop crash on yfinance DataFrames
...
def add_vwap(df):
    # FIX: use typical price (H+L+C)/3 instead of Close only
    tp         = (df['High'] + df['Low'] + df['Close']) / 3
